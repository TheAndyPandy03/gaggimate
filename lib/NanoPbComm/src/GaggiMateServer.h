...
class GaggiMateServer {
  public:
    ...
    using PumpCallback =
        std::function<void(uint8_t index, PumpControlMode mode, float power, float pressure, float flow)>;
    ...
    using AutotuneCallback =
        std::function<void(uint32_t testTime, uint32_t samples, uint32_t heaterWattage)>;
    ...
    using GrinderCalibrationCallback =
        std::function<void(int32_t rawFine,
                           int32_t rawCoarse,
                           uint32_t steps,
    ...
    void setSystemInfo(const String &hardware,
                       const String &version,
                       const gm::DeviceCapabilities &capabilities);
    ...
    gm::Payload buildSensorData(float temperature,
                                float pressure,
                                float puckFlow,
                                float pumpFlow,
                                ...
                                float grindPosition = 0.0f,
                                int32_t grinderPositionRaw = 0);
    ...
    // Send a pre-built payload / batch of payloads.
    ...
    // Fire-and-forget variants for high-rate telemetry.
    void sendUnreliable(const gm::Payload &payload) {
        _endpoint.sendUnreliable(payload);
    }

    void sendUnreliableBatch(const gm::Payload *payloads, size_t count) {
        _endpoint.sendUnreliable(payloads, count);
    }

    // Command registrations (display -> controller)
    void onPing(PingCallback cb) {
        _pingCb = std::move(cb);
    }

    void onBoilerControl(BoilerCallback cb) {
        _boilerCb = std::move(cb);
    }

    void onPumpControl(PumpCallback cb) {
        _pumpCb = std::move(cb);
    }

    void onRelayControl(RelayCallback cb) {
        _relayCb = std::move(cb);
    }

    void onPidSettings(PidCallback cb) {
        _pidCb = std::move(cb);
    }

    void onPumpSettings(PumpSettingsCallback cb) {
        _pumpSettingsCb = std::move(cb);
    }

    void onAutotune(AutotuneCallback cb) {
        _autotuneCb = std::move(cb);
    }

    void onPressureScale(PressureScaleCallback cb) {
        _pressureScaleCb = std::move(cb);
    }

    void onTare(TareCallback cb) {
        _tareCb = std::move(cb);
    }

    void onLedControl(LedCallback cb) {
        _ledCb = std::move(cb);
    }

    void onGrinderCalibration(GrinderCalibrationCallback cb) {
        _grinderCalibrationCb = std::move(cb);
    }
  ...
  private:
    ...
    LedCallback _ledCb;
    GrinderCalibrationCallback _grinderCalibrationCb;

    void registerHandlers();
    void pushSystemInfo();

    // Drives the endpoint send pump / retransmit independently of the
    // controller's slower main loop.
